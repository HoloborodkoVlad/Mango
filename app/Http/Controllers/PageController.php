...
class PageController extends Controller
{
    ...
    {
        $loai = ProductType::all();
        $tangx= DB::select("SELECT * FROM products ORDER BY unit_price DESC  "); 
        return view('page.loai_all', ['all'=>$tangx, 'listcategory'=>$loai]);
    }
    public function getLow()
    {
       $loai = ProductType::all();
        $giamx= DB::select("SELECT * FROM products ORDER BY unit_price ASC  "); 
...
// dd($cart);
    	...
    	Session::forget('cart');
    	return redirect()->back()->with('thongbao', "Замовлення успішне");
